...
export default function App() {
  ...
  const exportToCSV = (data: LowStockProduct[], filename: string) => {
    ...
    const csvContent = [
      headers.join(","),
      ...data.map(p => `${p.name},${p.category},${p.currentStock},${p.reorderLevel},${p.unit},${p.status}`)
    ].join("\n");
  ...
  const exportToPDF = (data: LowStockProduct[], filename: string) => {
    ...
    autoTable(doc, {
      ...
      body: data.map(p => [p.name, p.category, p.currentStock, p.reorderLevel, p.unit, p.status]),
      ...
      <div className="  mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
            ...
            <p className="text-4xl font-bold text-red-600 mt-2">{filteredProducts.filter(p => p.status === 'Critical').length}</p>
          </div>
          <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-blue-600 text-white border-b border-gray-200">
                <tr>
                  ...
                  <th className="p-5 font-semibold">Reorder Level</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {paginatedProducts.length > 0 ? paginatedProducts.map((item) => (
                  <tr key={item.id} className="hover:bg-orange-50/30 transition-colors group">
                    <td className="p-5 font-medium text-slate-900">{item.name}</td>
                    <td className="p-5 text-gray-500">{item.category}</td>
                    <td className="p-5">
                      <span className={`font-bold ${item.status === 'Critical' ? 'text-red-600' : 'text-yellow-600'}`}>
                        {item.currentStock}
                      </span>
                    </td>
                    <td className="p-5 text-gray-500">{item.reorderLevel}</td>
                    <td className="p-5 text-gray-500">{item.unit}</td>
                    <td className="p-5">
                    ...
                    {["Groups", "Individuals", "Custom Emails"].map((tab) => (
                      <button
                        ...
                        className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-all ${
                          emailTo === tab ? "bg-white text-orange-600 shadow-sm" : "text-gray-500 hover:text-gray-700"
                        }`}
                      >
                        {tab}
                      </button>
