// components/bulk-upload/BulkUploadHeader.tsx
import { motion } from 'framer-motion';
import { Package } from 'lucide-react';

export function BulkUploadHeader() {
    return (
        <header className="bg-blue-600 rounded-2xl px-6 py-8 flex justify-between items-center shadow-lg relative overflow-hidden">
            ...
            <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="relative z-10">
                <h1 className="text-5xl font-black text-white tracking-tighter uppercase italic">Bulk products</h1>
            </motion.div>
            <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-white relative z-10">
